...
public class Game {
    ...
    public void startBetweenTurns() {
        this.phase = TurnPhase.BETWEEN_TURNS;
    }
    ...
    public void swapTurnPlayer() {
        if (players.size() < 2 || currentTurnPlayer == null) {
            return;
        }
        UUID playerA = players.get(0).playerId();
        UUID playerB = players.get(1).playerId();
        this.currentTurnPlayer = currentTurnPlayer.equals(playerA) ? playerB : playerA;
    }
