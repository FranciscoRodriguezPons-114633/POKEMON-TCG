...
public class VictoryService {
    ...
    public void applyKnockoutAndPrizes(GameAggregate game, UUID attacker, UUID defender) {
        ...
        int prizeLoss =
                game.activePokemonEx().getOrDefault(defender, false)
                        ? 2
                        : 1;

        int currentPrizes =
                game.prizeCardsRemaining().getOrDefault(attacker, 6);

        game.prizeCardsRemaining().put(
                attacker,
                Math.max(0, currentPrizes - prizeLoss)
        );
    ...
    public UUID checkWinner(GameAggregate game) {
        ...
        boolean p1PrizeWin =
                game.prizeCardsRemaining().getOrDefault(p1, 6) <= 0;

        boolean p2PrizeWin =
                game.prizeCardsRemaining().getOrDefault(p2, 6) <= 0;

        boolean p1DeckOut =
                game.deckCardsRemaining().getOrDefault(p1, 1) <= 0;

        boolean p2DeckOut =
                game.deckCardsRemaining().getOrDefault(p2, 1) <= 0;
